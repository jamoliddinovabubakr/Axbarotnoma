...
def get_allow_menus(role_id: int = None):
    ...
    allow_menus = Menu.objects.filter(pk__in=menus_id)
    return {'allow_menus': allow_menus}


@register.simple_tag()
def get_sub_menus():
    menus = Menu.objects.filter(type_menu=2).filter(pk__in=get_sub_menus_id).filter(status=True).order_by('menu_tr')
    return menus
